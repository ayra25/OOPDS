...
// Trimester 2610 - Virtual Machine and Assembly Language Interpreter
// File: TT4L_G01.cpp
// Group: TT4L_G01
// Members:
//   Member 1 - Muhammad Ezra : Register, GeneralRegister, FlagRegister classes
//   Member 2 - Umar : Memory, custom data structures (MyList, MyStack, MyQueue)
//   Member 3 - [YOUR NAME] : Instruction hierarchy (all instruction classes)
//   Member 4 - [YOUR NAME] : CPU class and Runner class (main execution engine)
// =============================================================================
...
public:
    ...
    int8_t read(int address) const {
        if (address < 0 || address > 63) return 0;
        return mem[address];
    }

    void write(int address, int8_t val) {
        if (address < 0 || address > 63) return;
        mem[address] = val;
    }

    int8_t getCell(int i) const { return mem[i]; }
};
